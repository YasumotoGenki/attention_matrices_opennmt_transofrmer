...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        top_attn = attns[:, 0, :, :].contiguous() # head index = 0
        src_tgt_attn = attns.clone() # (batch_size, head, T, src_len) T: could be 1 in the case of stepwise decoding or tgt_len
        attn_align = None
        if with_align:
        ...
        return output, top_attn, attn_align, src_tgt_attn
...
class TransformerDecoder(DecoderBase):
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            output, attn, attn_align, src_tgt_attn = layer(
                output,
                src_memory_bank,
                src_pad_mask,
                tgt_pad_mask,
            ...
            # (batch_size, head, T, src_len)
            # src_tgt_attns: (batch_size, head, 1, src_len) -> (batch_size, 1, heads, src_len)
            src_tgt_attn = src_tgt_attn.transpose(1, 2).contiguous()
            if src_tgt_attns is None:
                src_tgt_attns = src_tgt_attn.contiguous()
            else:
                src_tgt_attns = torch.cat((src_tgt_attns, src_tgt_attn), 1)

        output = self.layer_norm(output)
        dec_outs = output.transpose(0, 1).contiguous()
        attn = attn.transpose(0, 1).contiguous() # attn: (batch_num, 1, src_len) -> (1, batch_num, src_len)
        src_tgt_attns = src_tgt_attns.unsqueeze(0) # (1, batch_size, 6 (layer_num), heads, src_len)
        ...
        return dec_outs, attns, src_tgt_attns
