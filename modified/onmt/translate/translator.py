...
class Translator(object):
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                if attn_debug:
                    ...
                    src_tgt_attention = trans.src_tgt_attn[0]
                    layer_num = src_tgt_attention.shape[1]
                    head_num = src_tgt_attention.shape[2]
                    for i in range(layer_num):
                        for j in range(head_num):
                            output_txt = "layer: " + str(i+1) + " || head: " + str(j+1) + "\n"
                            attns = src_tgt_attention[:, i, j, :]
                            attns = attns.squeeze()
                            attns = attns.tolist()
                            ...
                            if self.logger:
                                ...
                                os.write(1, output_txt.encode('utf-8'))
                                os.write(1, output.encode('utf-8'))
                        attns = src_tgt_attention[:, i, :, :].squeeze()
                        output_txt = "layer: " + str(i+1) + " || average of heads" + "\n"
                        avg_attns = attns.mean(dim=1).tolist()
                        output = report_matrix(srcs, preds, avg_attns)
                        if self.logger:
                            self.logger.info(output_txt)
                            self.logger.info(output)
                        else:
    ...
    def _decode_and_generate(
        ...
        dec_out, dec_attn, src_tgt_attns = self.model.decoder(
        ...
        if not self.copy_attn:
            ...
            # returns [(batch_size x beam_size) , vocab ] when 1 step
            # or [ tgt_len, batch_size, vocab ] when full sentence
        return log_probs, attn, src_tgt_attns
    ...
    def _translate_batch_with_strategy(
        ...
        for step in range(decode_strategy.max_length):
            ...
            log_probs, attn, src_tgt_attns = self._decode_and_generate(
                decoder_input,
                memory_bank,
                batch,
                src_vocabs,
            ...
            decode_strategy.advance(log_probs, attn, src_tgt_attns)
        ...
        results["src_tgt_attn"] = decode_strategy.src_tgt_attn
        if self.report_align:
