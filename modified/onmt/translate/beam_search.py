...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn, src_tgt_attns):
        ...
        ### torch.div(self.topk_ids, vocab_size, out=self._batch_index) ### comment out
        self._batch_index = self.topk_ids // vocab_size # add to avoid error
        ...
        if self.return_attention or self._cov_pen:
            current_attn = attn.index_select(1, self.select_indices) # (1, batch_num, src_len) -> (1, batch_num, src_len)
            cur_src_tgt_attns = src_tgt_attns.index_select(1, self.select_indices)
            if step == 1:
                self.alive_attn = current_attn
                self.alive_src_tgt_attn = cur_src_tgt_attns # (1, batch_size, 6 (layer_num), heads, src_len)
                ...
                self.alive_src_tgt_attn = self.alive_src_tgt_attn.index_select(1, self.select_indices)
                self.alive_attn = torch.cat([self.alive_attn, current_attn], 0) # (step, left_batch_num, src_len)
                self.alive_src_tgt_attn = torch.cat([self.alive_src_tgt_attn, cur_src_tgt_attns], 0) # (step, left_batch_num, 6 (layer_num), heads, src_len)
                # update global state (step > 1)
                if self._cov_pen:
    ...
    def update_finished(self):
        ...
        layer_num = self.alive_src_tgt_attn.shape[2] # (step, batch_size, 6 (layer_num), heads, src_len)
        head_num = self.alive_src_tgt_attn.shape[3] # (step, batch_size, 6 (layer_num), heads, src_len)
        src_tgt_attention = (
            self.alive_src_tgt_attn.view(
                step - 1, _B_old, self.beam_size, layer_num, head_num, self.alive_src_tgt_attn.size(-1))
            if self.alive_src_tgt_attn is not None else None)
        ...
        for i in range(self.is_finished.size(0)):  # Batch level
            ...
            for j in finished_hyp:  # Beam level: finished beam j in batch i
                ...
                self.hypotheses[b].append((
                    ...
                    src_tgt_attention[:, i, j, :, :, :self.memory_lengths[i]]
                    if src_tgt_attention is not None else None))
            ...
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                ...
                for n, (score, pred, attn, src_tgt_attn) in enumerate(best_hyp):
                    if n >= self.n_best:
                        break
                    ...
                    self.src_tgt_attn[b].append(
                        src_tgt_attn if src_tgt_attn is not None else [])
            else:
            ...
            if self._cov_pen:
                ...
                if self._stepwise_cov_pen:
                    self._prev_penalty = self._prev_penalty.index_select(
                        0, non_finished)
        if self.alive_src_tgt_attn is not None:
            inp_seq_len = self.alive_src_tgt_attn.size(-1)
            self.alive_src_tgt_attn = src_tgt_attention.index_select(1, non_finished) \
                .view(step - 1, _B_new * self.beam_size, layer_num, head_num, inp_seq_len)
